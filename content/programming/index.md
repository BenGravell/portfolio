---
permalink: /programming
layout: page
title: Programming
show_sidebar: false
---

<div class="programming-layout">

  <div class="programming-section">
    <div class="programming-section-label">
      <span class="programming-section-name">Foundations</span>
    </div>
    <div class="programming-bento-grid">
      <a class="programming-bento-card" href="{{ '/programming/art-and-practice' | relative_url }}">
        <p class="programming-bento-title">Art &amp; Practice</p>
        <p class="programming-bento-desc">Philosophy and practice of programming — timeless principles for writing good software.</p>
      </a>
    </div>
  </div>

  <div class="programming-section">
    <div class="programming-section-label">
      <span class="programming-section-name">Languages</span>
    </div>
    <div class="programming-bento-grid">
      <a class="programming-bento-card" href="{{ '/programming/cpp' | relative_url }}">
        <p class="programming-bento-title">C++</p>
        <p class="programming-bento-desc">Libraries for numerical computing, linear algebra, and high-performance C++.</p>
      </a>
      <a class="programming-bento-card" href="{{ '/programming/python' | relative_url }}">
        <p class="programming-bento-title">Python</p>
        <p class="programming-bento-desc">Scientific computing, optimization, control, reinforcement learning, visualization, and type checking.</p>
      </a>
      <a class="programming-bento-card" href="{{ '/programming/matlab' | relative_url }}">
        <p class="programming-bento-title">MATLAB</p>
        <p class="programming-bento-desc">Optimization, control, system identification, and machine learning toolboxes.</p>
      </a>
      <a class="programming-bento-card" href="{{ '/programming/julia' | relative_url }}">
        <p class="programming-bento-title">Julia</p>
        <p class="programming-bento-desc">Numerical computing, graphs, visualization, and interop with MATLAB and Python.</p>
      </a>
      <a class="programming-bento-card" href="{{ '/programming/interoperability' | relative_url }}">
        <p class="programming-bento-title">Interoperability</p>
        <p class="programming-bento-desc">Tools for C++/Python bindings and high-performance Python compilation.</p>
      </a>
    </div>
  </div>

  <div class="programming-section">
    <div class="programming-section-label">
      <span class="programming-section-name">Tools &amp; Utilities</span>
    </div>
    <div class="programming-bento-grid">
      <a class="programming-bento-card" href="{{ '/programming/trajectory-optimization' | relative_url }}">
        <p class="programming-bento-title">Trajectory Optimization</p>
        <p class="programming-bento-desc">Gradient-based and sampling-based MPC software for robotics and control.</p>
      </a>
      <a class="programming-bento-card" href="{{ '/programming/benchmarks' | relative_url }}">
        <p class="programming-bento-title">Benchmarks &amp; Datasets</p>
        <p class="programming-bento-desc">Benchmark suites, simulation environments, and datasets for control, system identification, reinforcement learning, and machine learning.</p>
      </a>
      <a class="programming-bento-card" href="{{ '/programming/tools' | relative_url }}">
        <p class="programming-bento-title">Tools &amp; Utilities</p>
        <p class="programming-bento-desc">Computation engines, plotting and visualization tools, Python IDEs, and robotics education platforms.</p>
      </a>
    </div>
  </div>

</div>
